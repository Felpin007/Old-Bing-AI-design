...
import './App.css';
// index.css is imported globally in main.jsx
...
function App() {
  ...
  const [aiResponseCountInTopic, setAiResponseCountInTopic] = useState(0);
  ...
  const [selectedModel, setSelectedModel] = useState(() => localStorage.getItem('geminiModel') || 'gemini-2.0-flash');
  // Effect to update body class based on activeStyle
  useEffect(() => {
  ...
  // Effect to load model from localStorage on mount
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    setAiResponseCountInTopic(nextSequenceNumber); // Increment counter *after* adding placeholder
    setInputValue('');
    setIsLoading(true);
    ...
    try {
      ...
      if (!chatSessionRef.current) {
        // Format history from messages *before* the current user message
        const history = messages
          .filter(msg => msg.sender === 'user' || msg.sender === 'ai') // Ensure only valid roles
          .map(msg => ({
      ...
      // Send message using the chat session - STREAMING
      const stream = await chatSessionRef.current.sendMessageStream({
        message: currentInput,
      });

      let accumulatedText = ''; // To build the full text for potential history issues if needed

      // Iterate through the stream and update the last message
      for await (const chunk of stream) {
        const chunkText = chunk.text;
        accumulatedText += chunkText;
        // Update the text of the *last* message (the AI placeholder)
        setMessages(prevMessages => {
          const updatedMessages = [...prevMessages];
          const lastMessageIndex = updatedMessages.length - 1;
          if (lastMessageIndex >= 0 && updatedMessages[lastMessageIndex].sender === 'ai') {
            // Ensure sequenceNumber is preserved when updating text
      ...
      // No need for the final setMessages here as it was updated incrementally
      ...
      if (error.response && error.response.promptFeedback) {
          ...
          chatSessionRef.current = null; // Reset chat on history error
      }
      // Update the last message (placeholder) with the error message
      setMessages(prevMessages => {
          const updatedMessages = [...prevMessages];
          const lastMessageIndex = updatedMessages.length - 1;
           // Ensure there's a placeholder to update, otherwise add the error as new message
          if (lastMessageIndex >= 0 && updatedMessages[lastMessageIndex].sender === 'ai' && updatedMessages[lastMessageIndex].text === '') {
             // Ensure sequenceNumber is preserved when adding error text
             updatedMessages[lastMessageIndex] = {
               ...updatedMessages[lastMessageIndex],
               text: errorMessage
             };
          } else {
              // If something went wrong before placeholder was added or it was already filled
              // If adding a new error message, assign a sequence number (though it might be off if placeholder failed)
              ...
              setAiResponseCountInTopic(errorSequence); // Increment counter if adding new error message
          }
          return updatedMessages;
        });
    } finally {
      ...
      <div className="chat-messages">
        {messages.map((message, index) => (
          <div key={index} className={`message ${message.sender}`}>
            {message.sender === 'user' ? (
              ...
              // AI message: Render with footer structure
              <div className="ai-message-wrapper">
                <div className="ai-message-content">
                  <ReactMarkdown
                    remarkPlugins={[remarkGfm, remarkMath]}
                    rehypePlugins={[rehypeHighlight, rehypeKatex]}
                    // Pass the CodeBlock component to handle code rendering
                    components={{
                      code: CodeBlock,
                      pre: ({node, ...props}) => <>{props.children}</> // Garante que o CodeBlock seja chamado apenas para <code>
                    }}
                  >
                    {message.text}
                  </ReactMarkdown>
        ...
        <button
              ...
              onClick={() => {
                ...
                setAiResponseCountInTopic(0); // Reset AI response counter
                console.log("New topic started (input, messages, chat session, and AI count cleared)");
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                if (apiKey) {
                  ...
                  console.log("Saved API Key and Model to localStorage"); // Keep this log for user feedback
